...
class LSStudyGUI(QWidget):   
    def __init__(self, learnset_controller, learnset_obj):
        ...
        self.word_index = 0
        self.popup = None
        random.shuffle(self.learnset_obj.word_list) #shuffle words
        if self.learnset_obj.word_list >0:
            self.current_word = self.learnset_obj.word_list[self.word_index]
        else:
    ...
    def create_main_frame(self):
        ...
        self.img_pixmap = QPixmap('images\GGA_logo.png')
    ...
    def handle_previous_event(self):
        #no words
        if len(self.learnset_obj.word_list)== 0:
            self.create_popup("There are no words in the learnset.")
            return
        #only 1 word
        elif len(self.learnset_obj.word_list)== 1:
        ...
        #first word in list
            ...
            self.word_index = len(self.learnset_obj.word_list) -1
            self.current_word = self.learnset_obj.word_list[self.word_index]
        self.update()
    ...
    def handle_next_event(self):
        ...
        elif (self.word_index +1) < len(self.learnset_obj.word_list):
            self.word_index +=1
            self.current_word = self.learnset_obj.word_list[self.word_index]
        else:
            self.word_index =0
